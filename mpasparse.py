...
def p_locals_1(p):
    ...
    locals : local SEMI
    '''
    p[0] = Locals([p[1]], lineno=p.lineno(1))

def p_local(p):
    '''
    local : ID COLON TYPENAME 
    '''
    p[0] = Local(p[1], p[3],None, lineno=p.lineno(1))
...
def p_local_1(p):
    ...
    '''
    p[0] = Local(p[1], p[3], p[5], lineno=p.lineno(1))
...
def p_localError1(p):
    ...
    locals : locals local       
    '''
    error(p.lineno,"Mala definicion de local. Falta de ';'")
    pass
...
def p_localError3(p):
    ...
    '''
    error(p.lineno,"Tipo de dato no valido")
...
def p_fundecl_1(p):
    ...
    '''
    p[0] = Funcdecl(p[2], p[4], None, p[7], lineno=p.lineno(1)) 
...
def p_parm_declaration(p):
    ...
    '''
    p[0] = Parameters_Declaration(p[1], p[3],None, lineno=p.lineno(1))
...
def p_ifError1(p):
    ...
    '''
    error(p.lineno,"Falta sentencia 'THEN'" )
    pass
...
def p_ifError2(p):
    ...
    '''
    error(p.lineno,"Mal manejo de sentencia 'THEN'")
...
def p_whileError(p):
    ...
    '''
    error(p.lineno,"Falta sentencia 'DO' despues del la condicion")
...
def p_expression_group(p):
    ...
    '''
    p[0] = Group(p[2], lineno=p.lineno(1))
...
def p_cond(p):
    ...
    if( len(p) == 4):
        if(p[2] == 'or' ):
            p[0] = Relation('or', p[1], p[3], lineno=p.lineno(1))
        else:
            p[0] = Relation('and', p[1], p[3], lineno=p.lineno(1))
    elif (len(p) == 3):
...
def p_expression_2(p):
    ...
    '''
    p[0] = Cast(p[1],p[3], lineno=p.lineno(1))
...
def p_literal(p):
    ...
    literal : INTEGER
    '''
    p[0] = Literal("int",p[1], lineno=p.lineno(1))

def p_literal_1(p):
    '''
    literal : FLOAT
    '''
    p[0] = Literal("float",p[1], lineno=p.lineno(1))

def p_literal_2(p):
    '''
    literals : STRING
    '''
    p[0] = Literal("string",p[1], lineno=p.lineno(1))

def p_location(p):
    '''
    location : ID
    '''
    p[0] = Location(p[1],None, lineno=p.lineno(1))

def p_location_2(p):
    '''
    location : ID LBRACKET expression RBRACKET
    '''
    p[0] = Location(p[1],p[3], lineno=p.lineno(1))
